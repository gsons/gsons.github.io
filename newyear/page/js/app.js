...
$("#prev").click(function(){
    ...
    this.pg=new Page("page1");
    this.lantern=[];
    this.addItems=function(){
     	     this.pg.addBgItem(this.bg);
      	this.pg.addBgItem(this.snow);
         		for(var i=0;i<5;i++){
			this.lantern[i]=new Item("lantern"+i,"./image/lantern.png",65,62,i*80,10,"swingLR 5s linear infinite",0);
			this.pg.addElement(this.lantern[i]);
		}
        this.pg.addElement(this.text_1);
        this.pg.addElement(this.text_2);
        this.pg.addElement(this.text_3);
        this.pg.addElement(this.text_4);
        this.pg.addElement(this.cloud1);
        this.pg.addElement(this.cloud2);
     }
    this.show=function(){
        ...
        for(var i in this.lantern){
        	...
        	$("#page1"+" ."+this.lantern[i].mclass).show();
        }
    	sequence_active(this.pg,items,times,0);
    }
    this.addItems();
	 }
    ...
    this.pg=new Page("page2");
    this.addItems=function(){
      this.pg.addBgItem(this.bg);
      this.pg.addBgItem(this.snow);
       this.pg.addElement(this.imooc);
      this.pg.addElement(this.lantern);
    }
    this.addItems();
    this.show=function(){
    	$("#page2").show();
	...
	this.couplet_2=new Item("couplet_2","./image/couplet_2.png",57,460,320,115,"zoomInUp .5s linear",0);
     this.bg= new BgItem("bg","./image/p3_bg.jpg","120%","",0)
    this.snow= new BgItem("snow","./image/snow.gif","120%","",1);
    this.pg=new Page("page3");
     this.addItems=function(){
     	 this.pg.addBgItem(this.bg);
      	this.pg.addBgItem(this.snow);
     	var items=[this.text_1,this.text_2,this.text_3,this.text_4,
     	this.couplet1,this.couplet_1,this.couplet2,this.couplet_2];//定义item数组
     	for (var i in items ) {
     		this.pg.addElement(items[i]);
     	}
     }
	  this.show=function(){
	  	$("#page3").show();
	...
	function Page4(){
	 ...
	 this.people=new Item("people","./image/people.png",400,390,0,screen.height-screen.width*400/412,"zoomInUp 1.4s",0);
      ...
      this.addItems=function(){
        this.pg.addBgItem(this.bg);
      	...
      	this.pg.addElement(this.text_1);
	    this.pg.addElement(this.text_2);
	    this.pg.addElement(this.people);
	    this.pg.addElement(this.blessing);
	    for(var i=0;i<5;i++){
			this.lantern[i]=new Item("lantern"+i,"./image/lantern.png",65,62,i*80,10,"swingLR 5s linear infinite",0);
			this.pg.addElement(this.lantern[i]);
		}
		 $("#page4 .people").css({"top":"","bottom":"0rem"});
      }
      this.show=function(){
      	 $("#page4").show();
	 ...
	 this.p5_home=new Item("p5_home","./image/p5_home.png",400,200,0,540,"p5_home 5s linear forwards",22);
       this.bg= new BgItem("bg","./image/p5_bg.jpg","130%","",0);
        this.snow= new BgItem("snow","./image/snow.gif","100%","",1);
         this.pg_fire=[];
         this.pg=new Page("page5");
       this.addItems=function(){
       	    this.pg.addBgItem(this.bg);
       	    this.pg.addBgItem(this.snow);
       	    var mitems=[this.p5_home,this.car_l,this.car_r];
       	    for(var i in mitems){
       	    	this.pg.addElement(mitems[i]);
       	    }
       	    for(var i=0;i<12;i++){
	 ...
	 this.p6_home=new Item("p6_home","./image/p5_home.png",400,200,0,540,"",22);
	  this.bg= new BgItem("bg","./image/p5_bg.jpg","130%","",0);
         this.pg=new Page("page6");
      this.addItems=function(){
      	 this.pg.addBgItem(this.bg);
      	 this.pg.addElement(this.p6_home);
      	 $("#page6").append("<div id='myfireworks'></div>");
      	 $("#page6 .p6_home").css({"top":"","bottom":"0rem"});
      }
	 this.show=function(){
	 	$("#page6").show();
	 	$("#page6 .bg").show();
   ...
   this.hide=function(){
	...
	setTimeout(function(){
		$("#"+page.id+" ."+items[i].mclass).css("display","block");sequence_active(page,items,times,i+1);
	},times[i]);
   }
